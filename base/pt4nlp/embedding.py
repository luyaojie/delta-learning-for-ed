...
class Embeddings(nn.Module):
    ...
    def __init__(self,
        ...
        if feature_dicts is not None and len(feature_dicts) > 0 and len(feature_dims) > 0 and feature_dims[0] > 0:
    ...
    def init_model(self):
        for emb_table in self.emb_luts:
            print("Init embedding (%s, %s) with normal (mean=0, std=0.1)" % (emb_table.weight.data.size(0),
                                                                             emb_table.weight.data.size(1)))
            emb_table.weight.data.normal_(0, 0.1)
            emb_table.weight.data[Constants.PAD].zero_()

    def load_pretrained_vectors(self, emb_file, binary=True, normalize=False, oov='ignore'):
            ...
            for word in self.word_dict:
                ...
                if word not in vocab:
                    ...
                    nn.init.uniform_(pretrained[self.word_dict.lookup(word)],
                                     random_range[0].item(), random_range[1].item())
            pretrained[Constants.PAD].zero_()
    ...
    def weight_word_embedding(self, x):
        """
        Return the embeddings for words, weighted by x
        :param x:   inp (FloatTensor): batch x len x dict_size
        :return:
            batch x len x word_dim
        """
        assert x.dim() == 3
        assert x.size(2) == self.word_lookup_table.weight.size(0)
        return torch.matmul(x, self.word_lookup_table.weight)
...
class ConvolutionEmbedding(nn.Module):
    ...
    def __init__(self,
        ...
        self.feat_embedding = Embeddings(word_vec_size=feat_vec_size,
                                         dicts=feat_dict)

        self.cnn_encoder = MultiSizeCNNEncoder(feat_vec_size,
        ...
        self.output_size = self.cnn_encoder.output_size

    def forward(self, x, lengths):
        """
        :param x:       (batch, length, feat_length)
        :param lengths: (batch, length, )
        :return:
        """
        batch, sentence_length, feat_length = x.size()
        x_reshape = x.view(batch * sentence_length, feat_length)
        x_reshape_embedding = self.feat_embedding.forward(x_reshape)
        x_lengths = torch.sum(x_reshape != Constants.PAD, 1).long()
        x_conv = self.cnn_encoder.forward(x_reshape_embedding, x_lengths)
        return x_conv.view(batch, sentence_length, -1)
